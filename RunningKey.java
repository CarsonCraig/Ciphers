...
public class RunningKey {
    ...
    static Scanner in;
    static String runningKey = "";
    static String lengthenedRunningKey = "";
    static int[] numericRunningKey;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter a running key longer than the " + (performEncrypt ? "plaintext." : "ciphertext."));
        runningKey = in.nextLine();
        runningKey = formatString(runningKey);

        lengthenedRunningKey = generateRunningKey(runningKey, input);


        numericRunningKey = turnNumeric(lengthenedRunningKey);

        addedNumeric = addNumericStrings(numericInput, numericRunningKey);
    ...
    public static String generateRunningKey(String runningKey, String input) {
        String returnRunningKey = "";

        while (returnRunningKey.length() < input.length()) {
            returnRunningKey = returnRunningKey + runningKey;
        }

        returnRunningKey = returnRunningKey.substring(0, input.length());

        return returnRunningKey;
    }
    ...
    public static int[] addNumericStrings(int[] numericInput, int[] numericRunningKey) {

        int[] returnAddedNumeric = new int[numericInput.length];

        for (int i = 0; i < numericInput.length; i++) {
            if (performEncrypt) {
                returnAddedNumeric[i] = (numericInput[i] + numericRunningKey[i]) % 26;
            } else {
                returnAddedNumeric[i] = (numericInput[i] + (26 - numericRunningKey[i])) % 26;
            }
        }
